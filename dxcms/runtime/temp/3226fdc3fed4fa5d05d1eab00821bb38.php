<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:80:"D:\mumu\WWW\windowCms\public/../dxcms/app/admin\view\category\edit_category.html";i:1585985128;s:61:"D:\mumu\WWW\windowCms\dxcms\app\admin\view\common\_style.html";i:1585897035;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
	<head>
		<meta charset="UTF-8">
...
<script src="/dxcms/js/dx-template.js" type="text/javascript" charset="utf-8"></script>
	...
	<body>
		<div class="container-fluid dx-main">
			<form action="<?php echo url('category/update'); ?>" method="post" class="layui-form">
					...
					<div class="tab-content stystem-tabs-content">
						...
						<div role="tabpanel" class="tab-pane col-md-3 active" id="home">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" name="title" value="<?php echo $category['title']; ?>" id="title" autocomplete="off">
								<input type="hidden" name="pid" value="<?php echo $category['pid']; ?>">
							</div>

							<div class="form-group">
								<label for="keyword">英文名称</label>
								<input type="text" class="form-control" name="name" value="<?php echo $category['name']; ?>" id="keyword" autocomplete="off">
							</div>
							...
							<div class="form-group">
								...
								<div class="layui-upload" id="upload">
									...
									<img  id="demo1" src="<?php echo $category['cover_img']; ?>" alt="">
								</div>
								<input class="form-control" type="text" readonly name="cover_img" value="<?php echo $category['cover_img']; ?>" id="face">
							</div>

							<div class="form-group">
								<label for="beian">栏目类型</label>
								<select name="type" class="form-control">
									<option value="1" <?php if($category['type'] == 1): ?> selected <?php endif; ?> >文章列表</option>
									<option value="2" <?php if($category['type'] == 2): ?> selected <?php endif; ?> >封面频道</option>
									<option value="3" <?php if($category['type'] == 3): ?> selected <?php endif; ?> >站外跳转</option>
									<option value="4" <?php if($category['type'] == 4): ?> selected <?php endif; ?> >专题活动</option>
								</select>
							</div>

							<div class="form-group">
								<label for="filepath">栏目文件夹名称</label>
								<input type="text" class="form-control" name="cate_folder" value="<?php echo $category['cate_folder']; ?>" id="filepath" autocomplete="off">
							</div>

							<div class="form-group">
								<label for="filepath">浏览权限</label>
								<select name="readauth" class="form-control">
									<option value="0" <?php if($category['readauth'] == 0): ?> selected <?php endif; ?> >开放浏览</option>
									<option value="1" <?php if($category['readauth'] == 1): ?> selected <?php endif; ?>>注册会员</option>
									<option value="2" <?php if($category['readauth'] == 2): ?> selected <?php endif; ?>>初级会员</option>
									<option value="3" <?php if($category['readauth'] == 3): ?> selected <?php endif; ?>>中级会员</option>
									<option value="4" <?php if($category['readauth'] == 4): ?> selected <?php endif; ?>>高级会员</option>
								</select>
							</div>
						...
						<div role="tabpanel" class="tab-pane col-md-6" id="profile">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" name="meta_title" value="<?php echo $category['meta_title']; ?>" id="captcha_font1"
								 autocomplete="off">
							</div>
							
							<div class="form-group">
								<label for="captcha_font">SEO关键词</label>
								<input type="text" class="form-control" name="meta_key" value="<?php echo $category['meta_key']; ?>" id="captcha_font2"
								 autocomplete="off">
							</div>
							
							<div class="form-group">
								<label for="captcha_font">SEO描述</label>
								<input type="text" class="form-control" name="meta_desc" value="<?php echo $category['meta_desc']; ?>" id="captcha_font3"
								 autocomplete="off">
							</div>
						...
						<div role="tabpanel" class="tab-pane col-md-6" id="settings" style="height: 530px;overflow: hidden;">
							<div class="form-group">
								<label for="captcha_font">栏目内容用于单独页面使用</label>
								<textarea name="content" class="form-control" id="content"><?php echo $category['content']; ?></textarea>
							</div>
						</div>
						<!-- 栏目内容 End -->
